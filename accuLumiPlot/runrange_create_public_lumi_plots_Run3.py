...
def GetUnits(year, accel_mode, mode):
    ...
    if (cfg_parser.get("general", "units")):
        cfg_units = json.loads(cfg_parser.get("general", "units"))
...
def FormatCMSEnergy(beam_energy, accel_mode, year, include_units=True):
    ...
    if accel_mode == "PROTPHYS":
        ...
        if accel_mode == "ALLIONS":
            ...
            try:
                accel_mode_by_year = json.loads(cfg_parser.get("general", "accel_mode_by_year"))
...
def loadCertificationJSON(json_file_name):
    ...
    #full_object = json.loads(full_file_content[0])
    full_file_content=full_file.read().replace("\n","")
    #print str(full_file_content)
    full_object = json.loads(full_file_content)
...
if __name__ == "__main__":
    ...
    if (cfg_parser.get("general", "skip_years")):
        skip_years = json.loads(cfg_parser.get("general", "skip_years"))
    ...
    if (cfg_parser.get("general", "display_units")):
        display_units = json.loads(cfg_parser.get("general", "display_units"))
    ...
    if (cfg_parser.get("general", "data_scale_factor")):
        data_scale_factor = json.loads(cfg_parser.get("general", "data_scale_factor"))
    ...
    if (cfg_parser.get("general", "display_scale_factor")):
        display_scale_factor = json.loads(cfg_parser.get("general", "display_scale_factor"))
            ...
            for type in ["lin", "log"]:
                ...
                if sum(weights_del) > 0.:
                    ...
                    fig.suptitle(r"CMS Peak Luminosity Per Day, " \
                                 "%s, %d, %s %s" % \
                                 (particle_type_str, year, sqrts_str, cms_energy_str)).set_fontproperties(FONT_PROPS_SUPTITLE)
    ...
    if plot_multiple_years:
            ...
            for color_scheme_name in color_scheme_names:
                ...
                for type in ["lin", "log"]:
                    ...
                    if accel_mode == "IONPHYS":
                        sqrts_str = "$\mathbf{\sqrt{s_{NN}} =}$"
                    if (len(cms_energy_strings) > 1):
                        fig.suptitle(r"CMS Integrated Luminosity Delivered, %s" % particle_type_str).set_fontproperties(FONT_PROPS_SUPTITLE)
                    else:
                        fig.suptitle(r"CMS Integrated Luminosity Delivered, %s, %s %s" % \
                                     (particle_type_str, sqrts_str, cms_energy_str)).set_fontproperties(FONT_PROPS_SUPTITLE)
                    ax.set_title("Data included from %s to %s UTC \n" % \
        ...
        for color_scheme_name in color_scheme_names:
			...
			continue
                ...
                if len(cms_energy_strings) > 1:
                    fig.suptitle(r"CMS Peak Luminosity Per Day, %s" % particle_type_str).set_fontproperties(FONT_PROPS_SUPTITLE)
                else:
